<!-- ようこそ、CacinieP の世界へ -->

# 🎵 CacinieP — 語碼 Cace

> *Linguist Wants Tech — 一个用代码写诗的语言学爱好者*

![Profile Views](https://komarev.com/ghpvc/?username=CacinieP&color=39c5bb&style=flat-square)
![Followers](https://img.shields.io/github/followers/CacinieP?color=39c5bb&style=flat-square&label=Followers)
![Stars](https://img.shields.io/badge/dynamic/json?color=39c5bb&label=Stars&query=%24.stars&url=https://api.github-star-counter.workers.dev/user/CacinieP&style=flat-square)

---

### 🎤 About Me

- 🔭 现在在做：AI Agent / Computer Use / GUI Agent 调研
- 🌱 正在学：Rust · 编译器 · 系统架构
- 🎵 信仰：*用技术的温柔，让世界更有趣*
- 🍣 代表作：回転寿司タイムマスター
- 📫 联系：[linguistwantstech.com](https://linguistwantstech.com) · [简历](https://caciniep.github.io/resume/)

---

### 🎶 Tech Stack

**Languages:** `TypeScript` `Python` `Swift` `Rust` `Go` `C++` `Kotlin` `Dart`

**Frontend:** `React` `Vue` `Next.js` `SwiftUI` `Flutter` `Electron`

**AI/ML:** `LLM Agent` `MLX` `Gemma` `Whisper`

**Tools:** `Tauri` `TSF` `Git` `CI/CD`

---

### 🌟 Featured Projects

| Project | Description |
|---------|-------------|
| 🍣 [kaitensushi-timemaster](https://github.com/CacinieP/kaitensushi-timemaster) | 回転寿司タイムマスター — 克服拖延症 |
| 🤖 [AgentBench](https://github.com/CacinieP/AgentBench) | AI Agent 回归测试与质量保障平台 |
| 📁 [TagResorter](https://github.com/CacinieP/TagResorter) | LLM 驱动的标签索引文件整理工具 |
| ⌨️ [TeXada](https://github.com/CacinieP/TeXada) | Windows 原生 TeX 数学公式输入法 |
| ⏱️ [cace-timer](https://github.com/CacinieP/cace-timer) | Minimal time tracking CLI |
| 📝 [CICPA-Learning](https://github.com/CacinieP/CICPA-Learning) | 注册会计师开源学习手册 ⭐4 |

---

### 📊 GitHub Stats

![CacinieP's Stats](https://github-readme-stats.vercel.app/api?username=CacinieP&show_icons=true&theme=vue&title_color=39c5bb&text_color=2ea9a3&icon_color=39c5bb&bg_color=00000000&hide_border=true&count_private=true)

![CacinieP's Streak](https://github-readme-streak-stats.herokuapp.com?user=CacinieP&theme=highcontrast&hide_border=true&stroke=39c5bb&ring=39c5bb&fire=0ee8b7&currStreakLabel=39c5bb)

---

> *"Tell Your World"* — 初音ミク
> 
> *虚拟的声音，真实的感动。每一行代码，都是一首歌。* 🎵

**みくみくにしてあげる♪**
